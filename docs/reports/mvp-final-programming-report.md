...
- Bekannte Risiken: keine echte Auth/RBAC, kein echtes Payment, keine medizinische Validierung, SQLite als lokaler Demo-State, moderate transitive `npm audit`-Warnung, Mock-Fallbacks für Demo-Robustheit.

## 2. Umgesetzte Produktverbesserungen

- Backend: Neuer sicherer Dokumenten-Metadaten-Endpunkt, erweiterte Order-Statuswerte, minimale Audit-Events für Profil, Export, Delete, Order, Upload und Tracking.
- Frontend/API: `apiClient` kann Multipart-Demo-Uploads, Meal-Tracking und Hydration-Aktionen gegen den BFF ausfuehren.
- Security-/Medizin-Texte: Sichtbarer Onboarding-Hinweis wurde von pauschalem Verschluesselungsclaim auf `Demo-Schutz` und lokale API-Key-Grenze entschärft; veraltete Design-HTML-Texte wurden von Therapie-/KI-Auswertungsclaims auf Demo-Plan-/Metadatenformulierungen umgestellt.
- Dokumentenfluss: Onboarding prüft Dateien weiterhin clientseitig und sendet ausgewählte Dateien als Metadaten-Upload an das Backend. Keine OCR, keine medizinische Auswertung.
- Tracking: Dashboard-Meal-Box und `+250 ml` Hydration schreiben regulaer in den Backend-Demo-State; Fallback bleibt lokal.
- Checkout: Order-API bleibt regulaerer Pfad; Statusmodell enthält nun auch `pending` und `demo_failed`.
...
- Infra/ENV: Minimale GitHub Actions CI für Backend-Tests und Frontend Build/Typecheck/Lint ergänzt.

## 3. Backend/API-Status

Zentrale Module bleiben unverändert in ihrer Architektur: Patient Profile, Questionnaire Intake, Recommendation Engine, Nutrition Plan, Frontend BFF, Tracking, Order Handling und Analytics. Neu hinzugekommen ist `document_upload` als bewusst schmaler Modulbereich für sichere MVP-Dokument-Metadaten.

Neue/angepasste Endpunkte:

- `POST /api/documents/upload`: nimmt genau eine Datei als Multipart-Feld `file` entgegen, erlaubt PDF/JPG/PNG bis 10 MB, speichert keine Inhalte und gibt `uploaded_demo` zurück.
...
- `POST /api/orders`: unverändert ohne echtes Payment, aber mit erweitertem Statusmodell und Audit-Event.

Error Handling:

- 404/422 laufen weiter über `{"error": {"code", "message", "details"}}`.
...
| `/dashboard` | Nutrition Plan, Meal-Tracking und Hydration BFF | Ja | Nur einfache Demo-Aktionen, kein vollständiges Tagesjournal |
...
| --- | --- | --- | --- |
| `cd backend && python -m pip install -e .` | Ausgefuehrt | Grün | `python-multipart` für Upload installiert |
| `cd backend && python -m pytest app/tests` | `57 passed` | Grün | Neue Tests für Upload, Order, Tracking und Error Envelope |
| `cd frontend && npm install` | Ausgefuehrt | Grün | Keine Dependency-Änderung im Frontend |
...
| `cd frontend && npm run lint` | Ausgefuehrt | Grün | ESLint sauber |
...
| `git diff --check` | Ausgefuehrt | Grün | Keine Whitespace-Fehler |
...
- Dokumenten-Upload hat keine sichere produktive Dateispeicherung, keine Virenprüfung und keine OCR.
...
- Mock-Fallbacks bleiben für Demo-Robustheit aktiv und müssen als Fallback verstanden werden.
...
- Echtes Auth/RBAC: zu groß für eine sichere kleine Iteration; API-Key bleibt Demo-/Dev-Schutz.
...
- Docker: nicht ergänzt, weil kein vorhandenes Setup existierte und lokale Reproduzierbarkeit bereits dokumentiert ist.
- Vollständige Profil-UI: kein UI-Redesign; Backend-Anbindung bleibt funktional, aber nicht feldvollständig.

## 9. Nächster Entwicklungsplan

1. Echtes Auth-/Session-Konzept mit Rollen für Patient, Fachpersonal und Admin entwerfen.
2. Produktives Audit Logging mit unveränderbaren Events und Aufbewahrungsregeln.
3. Medizinische Regelvalidierung durch Fachpersonal dokumentieren und testbar versionieren.
4. Dokumentenpipeline mit sicherer Speicherung, Virenprüfung, Consent und optionaler OCR-Sandbox.
5. Payment Provider im Sandbox-Modus anbinden, ohne echte Zahlungsdaten in der App zu speichern.
6. Datenbankmigrationen und Postgres-Profil für produktionsnaehere Umgebung.
7. E2E-Smokes für Onboarding -> Checkout automatisieren.
8. Mock-Fallbacks per sichtbarem Demo-/Dev-Modus schaltbar machen.
9. Profil-UI feldvollständig gegen `PatientProfile`-Schema ausbauen.
10. Npm-Audit-Warnung erneut prüfen, sobald ein sicherer Next-15.x/PostCSS-Fix verfügbar ist.
