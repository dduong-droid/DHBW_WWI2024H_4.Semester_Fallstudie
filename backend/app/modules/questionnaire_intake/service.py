...
def derive_flags(
    *,
    ...
    appetite_low = questionnaire.nutrition_status.appetite_level in {"reduced", "minimal"}
    chemo_related_appetite_loss = oncology_context and appetite_low
    gut_sensitivity = bool(digestive_symptoms or intolerances)
    ...
    high_protein_need = post_op_recovery or "protein" in goals or "genesung" in goals
    anti_inflammatory_focus = post_op_recovery or "entz" in goals or "schmerzen" in goals
    immune_focus = questionnaire.recovery_indicators.infections_last_year >= 3 or "immun" in goals
    energy_support = high_fatigue or appetite_low or "leistung" in goals or "energie" in goals
...
def create_questionnaire_intake(payload: QuestionnaireIntakeCreate) -> QuestionnaireIntakeRecord:
    ...
    questionnaire_content = QuestionnaireContent(**payload.dict(exclude={"patient_id"}))
