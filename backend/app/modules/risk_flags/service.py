        ...
        if weight_loss_percent >= 5:
            flags.append(
                _flag(
                    ...
                    severity="high",
                    title="Auffaelliger Gewichtsverlust",
                    description="Der Gewichtsverlauf spricht fuer ein erhoehtes Ernaehrungsrisiko.",
                    triggered_by=["profile.weight_kg", "intake.personal_and_body_data.weight_kg"],
                    recommended_action="Professionelle Pruefung einplanen, bevor der Plan freigegeben wird.",
    ...
    if nutrition.appetite_level == "minimal":
        flags.append(
            _flag(
                ...
                recommended_action="Plan nur als Entwurf anzeigen und fachliche Pruefung markieren.",
                requires_review=True,
                blocks_plan=True,
            )
        )
    ...
    if nutrition.intake_change_vs_past == "significantly_less" or nutrition.meals_per_day <= 1:
        flags.append(
            _flag(
                ...
                description="Die angegebene Nahrungsaufnahme ist fuer automatische Empfehlungen kritisch.",
                triggered_by=["nutrition_status.intake_change_vs_past", "nutrition_status.meals_per_day"],
                recommended_action="Professionelle Pruefung und ggf. medizinische Ruecksprache empfehlen.",
                requires_review=True,
                blocks_plan=True,
            )
        )
    ...
    if _contains_any(list(symptoms), {"erbrechen", "vomiting"}):
        flags.append(
            _flag(
                ...
                description="Erbrechen ist ein kritisches Signal fuer die Ernaehrungsplanung.",
                triggered_by=["nutrition_status.digestive_symptoms"],
                recommended_action="Keine automatische Freigabe; fachliche oder medizinische Pruefung markieren.",
                requires_review=True,
                blocks_plan=True,
            )
        )
    ...
    if _contains_any(list(symptoms), {"durchfall", "diarrhea"}):
        flags.append(
            _flag(
                ...
                description="Verdauungsbeschwerden sollten bei Plan und Hydration beruecksichtigt werden.",
                triggered_by=["nutrition_status.digestive_symptoms"],
    ...
    if _contains_any(list(eating_difficulties), {"schluck", "swallow", "dysphagie"}):
        flags.append(
            _flag(
                ...
                description="Bei Schluckproblemen duerfen normale feste Rezepte nicht als sicher freigegeben werden.",
                triggered_by=["nutrition_status.eating_difficulties"],
                recommended_action="Plan blockieren oder nur als Entwurf mit professioneller Pruefung anzeigen.",
                requires_review=True,
                blocks_plan=True,
            )
        )
    ...
    if profile.allergies:
        flags.append(
            _flag(
                ...
                description="Allergien muessen bei Rezepten und Meal-Kits hart ausgeschlossen werden.",
    ...
    if intake.gut_health.food_intolerances:
        flags.append(
            _flag(
                ...
                severity="medium",
                title="Unvertraeglichkeiten angegeben",
                description="Unvertraeglichkeiten schraenken Rezepte und Meal-Kits ein.",
                triggered_by=["gut_health.food_intolerances"],
                recommended_action="Unvertraeglichkeiten beim Matching ausschliessen oder stark abwerten.",
                requires_review=False,
            )
        )
    ...
    if not habits.can_cook and not habits.receives_support_for_cooking:
        flags.append(
            _flag(
                ...
                severity="medium",
                title="Keine Kochmoeglichkeit oder Unterstuetzung",
                ...
                recommended_action="Einfache Rezepte, Meal-Kits oder Angehoerigenunterstuetzung priorisieren.",
                requires_review=False,
            )
        )
    ...
    if meds.medications or meds.supplements:
        flags.append(
            _flag(
                ...
                recommended_action="Hinweis anzeigen, dass keine Interaktionspruefung stattfindet.",
                requires_review=False,
            )
        )
    ...
    if not profile.known_conditions:
        flags.append(
            _flag(
                ...
                description="Ohne klaren Kontext ist eine spezifische Empfehlung nur eingeschraenkt belastbar.",
    ...
    if any(marker in notes for marker in _CRITICAL_TEXT_MARKERS):
        flags.append(
            _flag(
                ...
                description="Der Freitext enthaelt moeglicherweise kritische Hinweise.",
                triggered_by=["additional_notes", "profile.notes"],
                recommended_action="Freitext menschlich pruefen, keine automatische Freigabe.",
                requires_review=True,
                blocks_plan=True,
            )
        )
    ...
    if habits.fluid_intake_ml_per_day == 0 or nutrition.meals_per_day == 0:
        flags.append(
            _flag(
                ...
                severity="high",
                title="Unvollstaendige oder nicht belastbare Daten",
                description="Einige Angaben reichen fuer eine sichere Empfehlung nicht aus.",
