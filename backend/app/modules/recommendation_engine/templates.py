...
def build_detected_needs(flags: DerivedFlags) -> list[str]:
    ...
    if flags.gut_sensitivity:
        needs.append("Darmfreundliche und milde Ernährung")
    if flags.high_fatigue:
        needs.append("Unterstützung bei Erschoepfung und geringer Belastbarkeit")
    if flags.immune_focus:
        needs.append("Immunstärkender Fokus")
...
def build_summary(flags: DerivedFlags) -> str:
    ...
    if flags.gut_sensitivity:
        return "Der Schwerpunkt liegt auf darmfreundlicher, milder Ernährung mit guter Verträglichkeit."
    if flags.post_op_recovery:
...
def build_rationale_lines(flags: DerivedFlags, top_kit_names: list[str], warnings: list[str]) -> list[str]:
    lines: list[str] = []
    if flags.post_op_recovery:
        lines.append("Die Empfehlung priorisiert eiweiß- und regenerationsorientierte Optionen für die Genesungsphase.")
    if flags.chemo_related_appetite_loss:
        lines.append("Es wurden leicht verdauliche, sanfte Mahlzeiten mit kleinerer Hürde für den Appetit bevorzugt.")
    if flags.gut_sensitivity:
        lines.append("Verdauungssensible Optionen wurden bevorzugt und potenziell belastende Produkte zurückgestellt.")
    if flags.high_fatigue:
    ...
    if warnings:
        lines.append("Berücksichtigte Ausschluesse: " + ", ".join(warnings) + ".")
    if not lines:
        lines.append("Es wurde eine ausgewogene Standardempfehlung für die allgemeine Regeneration erstellt.")
    return lines
